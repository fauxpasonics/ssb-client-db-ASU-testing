...
*/
		SELECT
			...
			, CASE WHEN PrimaryAddressType = 'P' THEN LEFT(PrimaryAddressCountry,200)
			...
			/*Email*/
			, ISNULL(p.[PersonalEmail],P.BusinessEmail) AS [EmailPrimary]
			...
			, p.Tag AS [ExtAttribute31]
			, p.comments AS [ExtAttribute32]
			, p.donor_comments AS [ExtAttribute33] 
			...
			, NULL [AccountId]
			, 'TM-' + [p].[Patron] customer_matchkey
			, CAST(CASE WHEN FullName LIKE '%,%' THEN 0 ELSE 1 END AS BIT) IsBusiness
			
		FROM etl.vwTI_PatronMDM p		
		WHERE 1=1
		AND PacCreateDate > DATEADD(day,-10,GETDATE()) OR UpdatedDate > DATEADD(day,-3,GETDATE()) 
		--AND LEN(FullName) > 3
