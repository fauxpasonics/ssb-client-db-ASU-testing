...
/**********
Salesforce to DimCustomer Loading View

Created by AMeitin 2017/05/18

Added CompanyName - AMeitin 2017/05/23

Changed Matchkey to only look at Primary customers and switch to TM - AMeitin 2017/07/20
...
--base set
		...
		'ASU SFDC' AS [SourceDB]
		, 'ASU PC_SFDC Account' AS [SourceSystem]
		...
		/*AddressPrimary*/
            , BillingStreet AS [AddressPrimaryStreet]
			, BillingCity AS [AddressPrimaryCity] 
			, BillingState  AS [AddressPrimaryState] 
			, [BillingPostalCode] AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, BillingCountry AS [AddressPrimaryCountry] 
			...
			, a.PersonEmail  AS [EmailOne]
			, NULL AS [EmailTwo]

				/*Extended Attributes*/
			, a.[Type] AS [ExtAttribute1] --nvarchar(100)
			...
			, CASE WHEN a.SSB_CRMSYSTEM_SSID_Winner_SourceSystem__c = 'TM'
				THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](a.[SSB_CRMSYSTEM_SSID_Winner__c],':') WHERE position = 1) 
				ELSE NULL
				END AS customer_matchkey

--			select top 100 *
			FROM ASU_Reporting.prodcopy.account a (NOLOCK)
			WHERE 1=1
			AND a.isdeleted = 0
			AND a.CopyLoadDate > DATEADD(DAY,-3,GETDATE())
			--OR IsPersonAccount = 0
