...
CREATE PROCEDURE [dbo].[rptCustSeasonTicketFootballRenew3_2017_BkpPacVersion] 
    (
      @startDate DATE
    , @endDate DATE
...
FROM #ReportBase rb  
        INNER JOIN #eventHeader eventHeaderEvent  
                ON rb.ITEM   
                 = eventHeaderEvent.eventCode   
    WHERE ITEM LIKE 'F0[1-6]%'
     AND rb.E_PL NOT IN  ('21', '18', '22', '19') -- premium price levels
...
FROM #ReportBase rb  
        INNER JOIN #eventHeader eventHeaderEvent  
                ON rb.ITEM   
                 = eventHeaderEvent.eventCode   
    WHERE ITEM LIKE 'F0[1-6]%'
     AND rb.E_PL IN  ('21', '18', '22') -- premium price levels
     AND rb.I_PRICE > 0
AND ( rb.PAIDTOTAL > 0 )
GROUP BY rb.ITEM 
) SuiteSet ON 
