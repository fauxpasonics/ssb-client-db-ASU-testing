...
CREATE  PROCEDURE [dbo].[rptCustSeasonTicketWBRenew2_2017Prod] 
    (
      @startDate DATE
    , @endDate DATE
...
--------------------------Build Price Level Map--------------------------------------------
 ...
 SELECT DISTINCT x.SeasonYear
        ...
        FROM    ( SELECT    fts.SeasonYear
								,CASE WHEN fts.PC1 = 'A' THEN 'Floor Sideline'
                                 WHEN fts.PC1 = 'B' THEN 'Floor Baseline'
                                 WHEN fts.PC1 = 'C' THEN 'Reserved'
                                 WHEN fts.PC1 = 'D' THEN 'General Admission'
                                 ELSE 'Unknown'
                            END AS PL_CLASS
                          , fts.PC1
...
UNION 
  SELECT DISTINCT x.SeasonYear
			,x.PL_CLASS
              , x.PC1
        FROM    ( SELECT    fts.SeasonYear
								,CASE WHEN fts.PriceLevelCode = 1 THEN 'Floor Sideline'
                                 WHEN fts.PriceLevelCode = 2 THEN 'Floor Baseline'
                                 WHEN fts.PriceLevelCode = 3 THEN 'Reserved'
                                 WHEN fts.PriceLevelCode = 4 THEN 'General Admission'
                                 ELSE 'Unknown'
                            END AS PL_CLASS
