...
CREATE PROCEDURE [dbo].[rptCustSelloutStrategy_Football_2015]
    (
...
DECLARE @curSeason VARCHAR (10) = 'F15'
...
Select 'F06',0,'1245928' UNION all
Select 'F07',0,'2112013'
                ...
                SELECT  'Total Distributed' AS Section 
						...
						LEFT JOIN ( SELECT  rb.SEASON 
											...
											, CASE WHEN item IN ('FSS','FSSUITE') AND rb.ORDTOTAL > 0
												   THEN 'Season Suites' 
												   WHEN item LIKE 'F0[1-7]' AND rb.ORDTOTAL > 0
...
/* ************************************************** #Output ************************************************************ */
		   ...
		   FROM(SELECT * FROM #Results
				...
				SELECT  'Recent Sales Velocity' AS Section
                      ...
                      , '121' AS Ticket_Type_Sort_Order
                FROM    #Results results
				WHERE subsection1 = 'Sold Tickets'
					  AND results.ORDER_DATE BETWEEN DATEADD(DAY,-7,GETDATE()) AND GETDATE() 
				GROUP BY  results.EventCode
						 , results.EventName
						 , results.EventDate
						 , results.Ticket_Type
				...
				SELECT  'Recent Sales Velocity' AS Section
                      ...
                      , SUM(results.Revenue) AS Revenue
                      , 12 AS Section_Sort_Order
                      , '122' AS Ticket_Type_Sort_Order
                FROM    #Results results
				WHERE subsection1 = 'Sold Tickets'
					  AND results.ORDER_DATE BETWEEN DATEADD(DAY,-14,GETDATE()) AND GETDATE() 
				GROUP BY  results.EventCode
						 , results.EventName
						 , results.EventDate
						 , results.Ticket_Type
				...
				SELECT  'Recent Sales Velocity' AS Section
                      ...
                      , SUM(results.Revenue) AS Revenue
                      , 12 AS Section_Sort_Order
                      , '123' AS Ticket_Type_Sort_Order
                FROM    #Results results
				WHERE subsection1 = 'Sold Tickets'
					  AND results.ORDER_DATE BETWEEN DATEADD(DAY,-30,GETDATE()) AND GETDATE() 
				GROUP BY  results.EventCode
						 , results.EventName
						 , results.EventDate
						 , results.Ticket_Type
...
/* ************************************************** Displayed Output ************************************************************ */
   ...
   SELECT  Section
        ...
        GROUP BY  Section
                ...
                , Section_Sort_Order
                , Ticket_Type_Sort_Order
        ORDER BY section_sort_order,ticket_type_sort_order
