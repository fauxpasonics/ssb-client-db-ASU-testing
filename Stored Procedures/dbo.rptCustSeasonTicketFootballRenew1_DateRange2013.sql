...
CREATE proc [dbo].[rptCustSeasonTicketFootballRenew1_DateRange2013]
as
...
select 
	tkOdet.Season
	,'Mini Packs FSE'
	,SUM(tkOdet.I_OQTY)/2 as  QTY  --Divde by 2 for 2012 season since there are 6 home games	
	,SUM(tkOdet.I_OQTY * (tkOdet.I_PRICE - tkOdet.I_DAMT)) AMT
from tk_odet tkOdet with (nolock)
where tkOdet.season in (@curSeason, @prevSeason)
...
from tk_odet tkOdet
where tkOdet.season in (@prevSeason)
	and tkOdet.ITEM in ('SUITE')  --WAS FSAF 
group by tkOdet.Season
UNION ALL
select
	tkOdet.Season
