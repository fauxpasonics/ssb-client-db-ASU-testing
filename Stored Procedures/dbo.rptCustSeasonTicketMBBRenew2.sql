...
-- Reviewed By: 
-- Reviewed Date: 
-- Description: ASU MBB HOB 
-- =============================================
 
/***** Revision History
 
Payton Soicher on 2018-07-23: Changed Qty Logic and added join to SeasonYear in PL Map, Reviewed By: Abbey M - 2018-07-23
 
*****/
/*************************************************************************************/
CREATE PROCEDURE [dbo].[rptCustSeasonTicketMBBRenew2] 
    (
      @startDate DATE
    , @endDate DATE
...
--------------------------Build Price Level Map--------------------------------------------
 ...
 SELECT DISTINCT x.SeasonYear
        ...
        FROM    ( SELECT    fts.SeasonYear
                  ...
                  WHERE     1 = 1
                            AND fts.SeasonYear = @curSeason AND fts.Sport = 'MBB'
...
UNION 
  SELECT DISTINCT x.SeasonYear
        ...
        FROM    ( SELECT    fts.SeasonYear
                  ...
                  FROM      ro.vw_FactTicketSalesBase_All fts
                  WHERE     1 = 1
                            AND fts.SeasonYear = @prevSeason AND fts.Sport = 'MBB'
							AND fts.TicketTypeCode = 'FS'
                  GROUP BY  fts.SeasonYear
							,fts.PC1
                ) x
...
FROM #ReportBase rb
LEFT JOIN #PriceLevelMap plm ON rb.PC1 = plm.PC1 AND rb.SeasonYear = plm.SeasonYear
...
SELECT 
      ...
      ,SUM(RevenueTotal) AMT
FROM #ReportBase rb
LEFT JOIN #PriceLevelMap plm ON rb.PriceLevelCode = plm.PC1 AND rb.SeasonYear = plm.SeasonYear
