...
CREATE PROCEDURE [dbo].[rptCustSeasonTicketSoftballRenew2_2019Prod] 
(
  @startDate DATE
, @endDate DATE
...
--------------------------Build Price Level Map--------------------------------------------
  SELECT DISTINCT x.SeasonYear
        ...
        FROM    ( SELECT    fts.SeasonYear
								,CASE WHEN fts.PC1 = 'A'	THEN 'Premium Field'
                                 WHEN fts.PC1 = 'B'			THEN 'Field'
                                 WHEN fts.PC1 = 'C'			THEN 'Bleacher Reserved'
                                 WHEN fts.PC1 = 'D'			THEN 'Students'
								 WHEN fts.PC1 = 'E'			THEN 'General Admission'
                                 ELSE 'Unknown'
                            END AS PL_CLASS
                          , fts.PC1
...
SELECT SeasonYear, Sport, DimSeasonId, TicketingAccountId, DimItemId, DimPriceCodeId, DimPriceTypeId
	, DimPlanTypeId, PC1, PC3, PriceLevelCode, TicketTypeCode, TicketTypeClass, IsComp, TransDateTime
	, SUM(QtySeatRenewable) QtySeatRenewable, SUM(RevenueTotal) RevenueTotal, SUM(PaidAmount) PaidAmount
FROM  [ro].[vw_FactTicketSalesBase_All] fts 
WHERE  1=1
...
GROUP BY SeasonYear, Sport, DimSeasonId, TicketingAccountId, DimItemId, DimPriceCodeId
	, DimPriceTypeId, DimPlanTypeId, PC1, PC3, PriceLevelCode, TicketTypeCode
...
FROM (
		SELECT DISTINCT PL_CLASS
		FROM #seasonsummary
	) PL   
LEFT OUTER JOIN (
		SELECT PL_CLASS, SUM(QTY) AS bbPrevQty, SUM(AMT) AS bbPrevAmt 
		FROM #seasonsummary 
...
LEFT OUTER JOIN (
		SELECT PL_CLASS, SUM(QTY) AS bbCurNewQty, SUM(AMT) AS bbCurNewAmt 
		FROM #seasonsummary 
		WHERE season IN (@curSeason)	 
			AND renewal = '0' 
		GROUP BY PL_CLASS
