...
--EXEC rptCustSeasonTicketFootballRenew1_2015Prod
--exec  [dbo].[rptCustSeasonTicketFootballRenew1_2015ProdReportBaseTest] 
--exec rptCustSeasonTicketFootballRenew1_2015Build
CREATE PROCEDURE [dbo].[rptCustSeasonTicketFootballRenew1_2017_bkpPacVersion]
    (
      @startDate DATE
    , @endDate DATE
...
Select 'Season', '5205900' UNION ALL
Select 'Season Suite', '0'   UNION ALL
Select 'Single Game Tickets', '4400000' UNION ALL
...
where ITEM = 'FS' AND SEASON in (@prevSeason, @curSeason)
    AND ( PAIDTOTAL > 0)
	 --OR CUSTOMER in ('152760','164043','186078' ) )
    --AND CUSTOMER <> '137398' -- removed 2/27/2017 ameitin
...
from #ReportBase rb 
    ...
    and ITEM like 'F0[1-6]'
     and rb.E_PL not in  ('19') -- deals with single game suites for now 
...
select 
    ...
    ,'Single Game Tickets'
,sum(ORDQTY) as Qty
,sum((ORDQTY * (CASE  WHEN ITEM IN ('F01', 'F02') AND E_PL = '13' THEN 30
					 WHEN ITEM IN ('F01', 'F02') AND E_PL = '16' THEN 20
                     WHEN ITEM = 'F03' THEN 50
                     WHEN ITEM = 'F04' THEN 50  
                     WHEN ITEM = 'F05' THEN 50
