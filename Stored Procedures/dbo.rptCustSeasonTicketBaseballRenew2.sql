...
CREATE PROCEDURE [dbo].[rptCustSeasonTicketBaseballRenew2] 
(
@startDate DATE
,@endDate DATE
...
-- Reviewed Date: ??
-- Description: ASU Baseball HOB Report Sproc 2 of 3
-- =============================================
 ...
 SELECT DISTINCT x.SeasonYear
        ...
        FROM    ( SELECT    fts.SeasonYear
                 ...
                 FROM      ro.vw_FactTicketSalesBase_All fts
                  WHERE     1 = 1
                            AND fts.SeasonYear = LEFT(@CurSeason,4) AND fts.Sport = @Sport
...
UNION 
  ...
  SELECT DISTINCT x.SeasonYear
        ...
        FROM    ( SELECT    fts.SeasonYear
                  ...
                  FROM      ro.vw_FactTicketSalesBase_All fts
                  WHERE     1 = 1
                            AND fts.SeasonYear = LEFT(@prevSeason,4) AND fts.Sport = @Sport
							AND fts.TicketTypeCode = 'FS'
                  GROUP BY  fts.SeasonYear
							,fts.PriceLevelCode
                ) x
...
where rb.SeasonYear in (LEFT(@curSeason,4))
    AND (PaidAmount > 0 OR PC3 = 'Z') --Sponsor Tickets
...
where rb.SeasonYear in (LEFT(@prevSeason,4))
    AND (PaidAmount > 0 OR PC3 = 'Z') --Sponsor Tickets
GROUP BY    rb.SeasonYear, rb.DimPlanTypeId, PL_CLASS
...
LEFT OUTER JOIN (SELECT PL_CLASS, SUM(QTY) AS bbPrevQty, SUM(AMT) AS bbPrevAmt 
					FROM #seasonsummary 
					WHERE season IN (LEFT(@prevSeason,4))				
...
LEFT OUTER JOIN (SELECT PL_CLASS
	, SUM(QTY) AS bbCurRenewQty, SUM(AMT) AS bbCurRenewAmt 
					FROM #seasonsummary 
					WHERE SEASON IN (LEFT(@curSeason,4))	  
					AND renewal = '1' 
