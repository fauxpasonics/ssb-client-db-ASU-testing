...
CREATE      PROCEDURE [dbo].[rptCustSeasonTicketSoftballRenew2_2015Prod] 
...
where rb.ITEM IN ('SBS')  AND rb.SEASON in (@curSeason, @prevSeason) --I'm not sure if this is right
                AND ( PAIDTOTAL > 0 )
...
select 
               ...
               ,isnull(bbCurRenew.bbCurRenewAmt,0) AS CYRenewAmt
                ...
                ,case when isnull(PL.PL_CLASS,'') in ('General Admission','Field Level','Reserved Level','Field','Premium Field') then 0 else 1 end as HideRow
