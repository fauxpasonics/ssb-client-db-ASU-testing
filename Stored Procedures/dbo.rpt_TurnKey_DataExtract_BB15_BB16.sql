...
CREATE PROCEDURE [dbo].[rpt_TurnKey_DataExtract_BB15_BB16]
AS
    BEGIN
...
--TRUNCATE TABLE  dbo.TI_TurnKey_DataExtract_BB16_BB15
...
-- Create #SalesBase --------------------------------------------------------------------------------------------------
        ...
        INSERT  INTO #SalesBase
                ...
                WHERE   1 = 1
                        AND tkTransItemEvent.SEASON IN ( 'BB16', 'BB15' )
                        ...
                        AND ( tkTrans.E_STAT NOT IN ( 'MI', 'MO', 'TO', 'TI',
						 ...
						 AND tkTransItem.Item <> 'FF' --remove FinalFour
						 AND tkTransItem.Item <> 'PAC12' --remove Pac-12
						 AND tkTransItem.Item NOT LIKE 'S%' --remove Pac-12
						...
						AND tkTransItem.I_PT <> 'S' --remove Students
						--AND tkTransItem.I_PT <> 'IN' --remove NBA Scouts
...
/******************************************* Output *********************************************************************************/
        ...
        SELECT  sb.SEASON
              ...
              INTO dbo.TI_TurnKey_DataExtract_BB16_BB17
        FROM    #SalesBase sb
