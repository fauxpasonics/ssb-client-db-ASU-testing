...
-- Reviewed By: 
-- Reviewed Date: 
-- Description: ASU Football HOB Top Summary Section
-- =============================================
...
CREATE PROCEDURE [dbo].[rptCustSeasonTicketFootballRenew1_2018]
    (
      @startDate DATE
    , @endDate DATE
...
SELECT 
 ...
 ,QtySeatFSE
 ,QtySeatRenewable
 --, CASE WHEN fts.DimEventId <= 0 AND fts.DimPlanId > 0 THEN QtySeat ELSE QtySeatRenewable END -- Payton Soicher 7/23/2018
 ,RevenueTotal  
 ,TransDateTime 
 ,PaidAmount
...
( SELECT 
     SeasonYear
     ,'Season Suite' SaleTypeName
     ,CASE WHEN ItemCode = '18FBFSST' THEN 0 ELSE COUNT(ItemCode)/6 END AS QTY
     ,SUM(RevenueTotal)  AMT
FROM #ReportBase 
where  TicketTypeCode = 'FSS'
       AND PaidAmount > 0 
