...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [etl].[SFMC_ods_Surveys]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM stg.SFMC_Surveys),'0');	
...
SELECT CAST(NULL AS BINARY(32)) ETL_DeltaHashKey, ETL_SourceFileName, ClientID, SendID, SubscriberKey, EmailAddress, SubscriberID, ListID, EventDate, EventType, Question, Answer, BatchID, TriggeredSendExternalKey
INTO #SrcData
FROM stg.SFMC_Surveys
...
MERGE ods.SFMC_Surveys AS myTarget
...
WHEN MATCHED AND (
	 ...
	 OR ISNULL(mySource.Question,'') <> ISNULL(myTarget.Question,'') OR ISNULL(mySource.Answer,'') <> ISNULL(myTarget.Answer,'') 
)
THEN UPDATE SET
     ...
     ,myTarget.[Question] = mySource.[Question]
     ,myTarget.[Answer] = mySource.[Answer]
...
DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SFMC_Surveys WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SFMC_Surveys WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
