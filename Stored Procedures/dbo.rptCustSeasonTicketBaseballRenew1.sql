...
CREATE PROCEDURE [dbo].[rptCustSeasonTicketBaseballRenew1] 
(
  @startDate DATE
, @endDate DATE
...
-- Reviewed Date: ??
-- Description: ASU Baseball HOB Report Sproc 1 of 3
-- =============================================
...
DECLARE @prevSeason varchar(50) = (cast(@curSeason as int)-1) --'2018'
Set @Sport = 'Baseball'
...
WHERE  1=1
AND fts.Sport = @Sport
AND (fts.SeasonYear = LEFT(@prevSeason, 4) 
...
WHERE TicketTypeCode = 'MINI'
    AND (PaidAmount > 0 OR PC3 = 'Z') --Sponsor Tickets
	AND Seasonyear = LEFT(@curSeason, 4)
	AND ItemCode <> CONCAT(RIGHT(@curSeason,2),'BBFLEX') -- update each year
GROUP BY SeasonYear   
...
SELECT  ---separated Flex Plans to account for FSE Calc 
	 ...
	 ,SUM(RevenueTotal) AMT
FROM #ReportBase 
WHERE ItemCode = CONCAT(RIGHT(@curSeason,2),'BBFLEX') -- update each year
    AND (PaidAmount > 0 OR PC3 = 'Z') --Sponsor Tickets
...
WHERE TicketTypeCode = 'MINI'
    AND (PaidAmount > 0) 
	AND Seasonyear = LEFT(@prevSeason, 4)
GROUP BY SeasonYear 
...
AS prevAmt FROM @seasonsummary a WHERE CAST(RIGHT(season,2) AS INT)
 = RIGHT(LEFT(@prevseason,4),2) GROUP BY saleTypeName) PrevYR
LEFT OUTER JOIN (SELECT saleTypeName
				, SUM(qty) AS curQty
				, SUM(amt) AS curAmt 
				FROM @seasonsummary a WHERE CAST(RIGHT(season,2) AS INT) 
 = RIGHT(LEFT(@curseason,4),2) GROUP BY saleTypeName) CurYR
