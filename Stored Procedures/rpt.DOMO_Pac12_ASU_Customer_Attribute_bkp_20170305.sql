...
CREATE PROCEDURE [rpt].[DOMO_Pac12_ASU_Customer_Attribute_bkp_20170305]
AS
    BEGIN
        ...
        INTO    #Membership
        FROM    #ID ID
                JOIN dbo.dimcustomerssbid ssbid ON ID.GIFT_CLUB_ID_NUMBER = ssbid.SSID
                                                   AND ssbid.SourceSystem = 'Advance ASU'
                LEFT JOIN ( SELECT DISTINCT
                                    Clubs.GIFT_CLUB_ID_NUMBER
                                  , GIFT_CLUB_DESC
                                  , QUALIFIED_AMOUNT
                            FROM    dbo.FD_SDC_GIFT_CLUBS Clubs
        ...
        INSERT  INTO [etl].[DOMO_Pac12_ExportFile_ASU_Customer_Attribute]
                SELECT  COALESCE(ssbid.SSB_CRMSYSTEM_CONTACT_ID, Membership.SSB_CRMSYSTEM_CONTACT_ID) SSB_CRMSYSTEM_CONTACT_ID
