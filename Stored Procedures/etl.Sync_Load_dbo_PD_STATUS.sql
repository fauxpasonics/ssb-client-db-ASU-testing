...
CREATE PROCEDURE [etl].[Sync_Load_dbo_PD_STATUS]
(
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.Sync_TI_PD_STATUS),'0');	
...
,  ETLSID, STATUS, NAME, LAST_USER, LAST_DATETIME, ZID, SOURCE_ID, EXPORT_DATETIME
INTO #SrcData
FROM (
	SELECT  ETLSID, STATUS, NAME, LAST_USER, LAST_DATETIME, ZID, SOURCE_ID, EXPORT_DATETIME
	, ROW_NUMBER() OVER(PARTITION BY ETLSID,STATUS ORDER BY ETL_Sync_ID) RowRank
	FROM src.Sync_TI_PD_STATUS
) a
WHERE RowRank = 1;
...
UPDATE #SrcData
SET ETL_Sync_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(ETLSID),'DBNULL_TEXT') + ISNULL(RTRIM(CONVERT(varchar(25),EXPORT_DATETIME)),'DBNULL_DATETIME') + ISNULL(RTRIM(CONVERT(varchar(25),LAST_DATETIME)),'DBNULL_DATETIME') + ISNULL(RTRIM(LAST_USER),'DBNULL_TEXT') + ISNULL(RTRIM(NAME),'DBNULL_TEXT') + ISNULL(RTRIM(SOURCE_ID),'DBNULL_TEXT') + ISNULL(RTRIM(STATUS),'DBNULL_TEXT') + ISNULL(RTRIM(ZID),'DBNULL_TEXT'));
...
CREATE NONCLUSTERED INDEX IDX_Load_Key ON #SrcData (ETLSID,STATUS);
...
MERGE dbo.PD_STATUS AS myTarget
USING (
	SELECT * FROM #SrcData
) AS mySource
ON myTarget.ETLSID = mySource.ETLSID and myTarget.STATUS = mySource.STATUS
