...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [etl].[SFMC_ods_Sent]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM stg.SFMC_Sent),'0');	
...
SELECT CAST(NULL AS BINARY(32)) ETL_DeltaHashKey, ETL_SourceFileName, ClientID, SendID, SubscriberKey, EmailAddress, SubscriberID, ListID, EventDate, EventType, BatchID, TriggeredSendExternalKey, CampaignID
INTO #SrcData
FROM stg.SFMC_Sent
...
UPDATE #SrcData
SET ETL_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(BatchID),'DBNULL_TEXT') + ISNULL(RTRIM(CampaignID),'DBNULL_TEXT') + ISNULL(RTRIM(ClientID),'DBNULL_TEXT') + ISNULL(RTRIM(EmailAddress),'DBNULL_TEXT') + ISNULL(RTRIM(ETL_SourceFileName),'DBNULL_TEXT') + ISNULL(RTRIM(EventDate),'DBNULL_TEXT') + ISNULL(RTRIM(EventType),'DBNULL_TEXT') + ISNULL(RTRIM(ListID),'DBNULL_TEXT') + ISNULL(RTRIM(SendID),'DBNULL_TEXT') + ISNULL(RTRIM(SubscriberID),'DBNULL_TEXT') + ISNULL(RTRIM(SubscriberKey),'DBNULL_TEXT') + ISNULL(RTRIM(TriggeredSendExternalKey),'DBNULL_TEXT'))
...
MERGE ods.SFMC_Sent AS myTarget
...
DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SFMC_Sent WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.SFMC_Sent WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
