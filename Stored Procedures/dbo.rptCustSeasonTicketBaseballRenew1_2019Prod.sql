...
CREATE PROCEDURE [dbo].[rptCustSeasonTicketBaseballRenew1_2019Prod] 
(
  @startDate DATE
, @endDate DATE
...
Set @curSeason = '2019'
Set @Sport = 'Baseball'
DECLARE @prevHomeGames AS NUMERIC = 34
...
INSERT INTO #ReportBase (SeasonYear, Sport, DimSeasonId, TicketingAccountId, DimItemId, ItemCode, PC3
	, DimPriceCodeId, DimPriceTypeId, DimPlanId, SectionName, TicketTypeCode, TicketTypeClass, IsComp
	, QtySeat, QtySeatFSE, QtySeatRenewable, RevenueTotal, TransDateTime, PaidAmount) 

SELECT SeasonYear, Sport, DimSeasonId, TicketingAccountId, DimItemId, ItemCode, PC3
	, DimPriceCodeId, DimPriceTypeId, DimPlanId, SectionName, TicketTypeCode, TicketTypeClass, IsComp
	, SUM(QtySeat), SUM(QtySeatFSE), SUM(QtySeatRenewable)
	, SUM(CASE WHEN ETL__SourceSystem = 'TM' THEN RevenueTotal
		ELSE RevenueTicket END) AS RevenueTotal
	, TransDateTime, SUM(PaidAmount)
...
GROUP BY SeasonYear, Sport, DimSeasonId, TicketingAccountId, DimItemId, ItemCode, PC3
...
WHERE TicketTypeCode = 'FS' 
	...
	AND SeasonYear IN (@curSeason, @prevSeason)
GROUP BY SeasonYear


UNION ALL 
-----------------------------------------------MINI Plans -------------------------------------------
SELECT SeasonYear, 'Mini Plans FSE', SUM(QTY) QTY, SUM(AMT) AMT
FROM (
		SELECT 
			  SeasonYear
		...
		WHERE TicketTypeCode = 'MINI'
			AND (PaidAmount > 0 OR PC3 = 'Z') --Sponsor Tickets
			AND ItemCode NOT LIKE '%BBFLEX' -- update each year
		GROUP BY SeasonYear   
		...
		SELECT  ---separated Flex Plans to account for FSE Calc 
			...
			, SUM(RevenueTotal) AMT
		FROM #ReportBase 
		WHERE ItemCode LIKE '%BBFLEX' -- update each year
			AND (PaidAmount > 0 OR PC3 = 'Z') --Sponsor Tickets
		GROUP BY SeasonYear   
	) cy
GROUP BY SeasonYear
...
SELECT rb.SeasonYear, 'Scout Card', SUM(rb.QtySeat) AS Qty, SUM(rb.RevenueTotal) AS Amt
FROM #ReportBase rb 
