...
*   5   01/24/2018      Tommy Dickinson         removed stg as an option because of non-payload stg tables in the db
***************************************************************************************************************************/
...
IF EXISTS (SELECT TOP 1 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA IN ('apietl') AND TABLE_TYPE = 'BASE TABLE' AND Table_Name NOT IN (SELECT DISTINCT Name FROM [etl].[JSON_Meta_Table]))
BEGIN

SET @SQL = '
INSERT INTO [etl].[JSON_Meta_Table] ([Schema],Name)
SELECT TABLE_SCHEMA,Table_Name FROM INFORMATION_SCHEMA.Tables WHERE TABLE_SCHEMA IN (''apietl'') AND TABLE_TYPE = ''BASE TABLE'' AND Table_Name NOT IN (SELECT DISTINCT Name FROM [etl].[JSON_Meta_Table])
