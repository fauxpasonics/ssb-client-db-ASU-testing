...
CREATE PROCEDURE [etl].[Dev_Load_FactTicketSalesEvents] 
( 
...
INSERT INTO dev.FactTicketSales 
(  
...
LEFT OUTER JOIN dev.DimSeason (NOLOCK) ds ON a.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = ds.ETL__SSID_SEASON AND ds.ETL__EndDate IS null 
LEFT OUTER JOIN dev.DimTicketCustomer (NOLOCK) dtc ON a.CUSTOMER = dtc.ETL__SSID_PATRON AND dtc.ETL__EndDate IS null 
LEFT OUTER JOIN dev.DimItem (NOLOCK) di ON a.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = di.ETL__SSID_SEASON AND a.ITEM COLLATE SQL_Latin1_General_CP1_CS_AS = di.ETL__SSID_ITEM AND di.ETL__EndDate IS null 
LEFT OUTER JOIN dev.DimEvent (NOLOCK) de ON a.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = de.ETL__SSID_SEASON AND a.[EVENT] COLLATE SQL_Latin1_General_CP1_CS_AS = de.ETL__SSID_EVENT AND de.ETL__EndDate IS null 
LEFT OUTER JOIN dev.DimPlan (NOLOCK) dplan ON a.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = dplan.ETL__SSID_SEASON AND a.[ITEM] COLLATE SQL_Latin1_General_CP1_CS_AS = dplan.ETL__SSID_ITEM AND dplan.ETL__EndDate IS null 
LEFT OUTER JOIN dev.DimArena (NOLOCK) da ON de.Arena COLLATE SQL_Latin1_General_CP1_CS_AS = da.ETL__SSID_FACILITY AND da.ETL__EndDate IS null 
...
LEFT OUTER JOIN dev.DimPriceLevel (NOLOCK) dpl ON a.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = dpl.ETL__SSID_SEASON AND tkItem.PTABLE COLLATE SQL_Latin1_General_CP1_CS_AS = dpl.ETL__SSID_PTable AND a.[E_PL] COLLATE SQL_Latin1_General_CP1_CS_AS = dpl.ETL__SSID_PL AND dpl.ETL__EndDate IS null 
LEFT OUTER JOIN dev.DimPriceType (NOLOCK) dpt ON a.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = dpt.ETL__SSID_SEASON and a.[E_PT] COLLATE SQL_Latin1_General_CP1_CS_AS = dpt.ETL__SSID_PRTYPE AND dpt.ETL__EndDate IS null 
